...
def distance(v1, v2, teleport_edges):
    ...
    if v2 in teleport_edges[v1]:
        return teleport_edges[v1][v2]
    if (v1.cell == v2.cell) or (not v1.cell.is_interior and not v2.cell.is_interior):
        return get_distance(v1.coords, v2.coords) * 30. / 100. / 3600.
    else:
        return None
...
def construct_graph(npcs, cells, source_location):
    ...
    vertices = set([source_location]) # todo needed?
    ...
    for c in cells:
        for r in c.references:
            ...
            if hasattr(r, 'destination'):
                ...
                teleport_edges[ref_pos][r.destination] = 0
                vertices.add(ref_pos)    
                vertices.add(r.destination)
            elif r.name in npc_ids:
                if npc_id_map[r.name].destinations:
                    ...
                    for dest in npc_id_map[r.name].destinations:
                        ...
                        if npc_id_map[r.name].class_name == 'Guild Guide':
                            ...
                            teleport_edges[ref_pos][dest] = get_distance(ref_pos.coords, dest.coords) / 16000.
                        vertices.add(dest)
    ...
    for v in vertices:
        try:
            teleport_edges[v][get_closest_from(divine_marker_locations, v)] = 0
        except AssertionError:
            pass
        
        try:
            teleport_edges[v][get_closest_from(temple_marker_locations, v)] = 0
        except AssertionError:
            pass
